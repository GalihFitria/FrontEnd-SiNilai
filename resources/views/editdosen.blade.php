...
<body class="bg-gray-100 p-6">
    <div class="flex items-center justify-center h-screen bg-blue-100">
        ...
        <div id="formModal" class="flex items-center justify-center bg-gray-800 bg-opacity-50 fixed inset-0">
            <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                <h2 class="text-xl font-bold mb-4 text-center">Edit Data Dosen</h2>
                <form action="{{ route('dosen.update', $dosen['nidn']) }}" method="post">
                    @csrf
                    @method('PUT')
                    <label class="block">NIDN</label>
                    <input type="text" name="nidn" value="{{$dosen['nidn']}}" id="editNIDN" class="border w-full p-2 mb-2 rounded">

                    <label class="block">Nama Dosen</label>
                    <input type="text" name="nama_dosen" value="{{$dosen['nama_dosen']}}" id="editNamaDosen" class="border w-full p-2 mb-2 rounded">

                    <div class="flex justify-center space-x-6 mt-4">
                        <a href="{{route('dosen.index')}}" class="bg-gray-400 text-white px-4 py-2 rounded-lg hover:bg-gray-500 transition duration-200 shadow-md">
                            Batal
                        </a>
    ...
    <div id="successModal" class="hidden fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96 text-center">
            ...
            <a href="datadosen" onclick="closeSuccessModal()" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">OK<a>
        </div>
    </div>
